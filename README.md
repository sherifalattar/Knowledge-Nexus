...
| **Codes · Global Atlas** | A map of the coding systems, and the gateway to the denial toolkit. |
| **Denial Walkthrough** | How a denial is *built* — group code → CARC → RARC → the DoH·Shafafiya layer → a claim's journey. |
| **Denial Navigator** | An offline, colour‑keyed taxonomy across thirteen reason families — searchable by code, family, or phrase. |
| **DoH Code Dictionary** | The Department of Health code workbook, rendered sheet‑by‑sheet with instant search. |
...
- The DoH dictionary reads `Codes.xlsx` at runtime — the one deliberate external dependency, kept isolated to that single page.
...
├─ codes-global-atlas.html         + denial toolkit gateway
├─ denial-codes-walkthrough.html
├─ denial-codes-semantic.html      the Navigator
├─ doh-codes-dictionary.html
...
└─ Codes.xlsx                      DoH workbook (read by the dictionary)
```
